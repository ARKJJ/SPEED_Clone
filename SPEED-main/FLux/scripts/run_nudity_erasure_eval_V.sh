...
MAX_NUM="${MAX_NUM:-}"
MAX_EDIT_TOKENS="${MAX_EDIT_TOKENS:-77}"
PARAMS="${PARAMS:-V}"

mkdir -p "${CHECKPOINT_DIR}" "${SAVE_ROOT_BASE}"

for r in $(seq 4 8); do
  RUN_NAME="erase_nudity_to_null_${PARAMS}_r${r}_t${TRACE_NUM_STEPS}"
  CKPT_PATH="${CHECKPOINT_DIR}/${RUN_NAME}.safetensors"
  SAVE_ROOT="${SAVE_ROOT_BASE}/nudity${PARAMS}${r}_tok${MAX_EDIT_TOKENS}"

  echo "========== [r=${r}] Erasing nudity with params=${PARAMS} =========="
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" python FLux/CE_Flux.py \
    ...
    --residual_scale "${r}" \
    --max_edit_tokens "${MAX_EDIT_TOKENS}"

  echo "========== [r=${r}] Sampling I2P edit images =========="
  sample_args=(
  ...
  echo "========== [r=${r}] Evaluating I2P with NudeNet =========="
