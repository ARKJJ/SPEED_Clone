...
for r in $(seq 7 15); do
  RUN_NAME="erase_snoopy_to_null_${PARAMS}_r${r}_t${TRACE_NUM_STEPS}"
  CKPT_PATH="${CHECKPOINT_DIR}/${RUN_NAME}.safetensors"
  SAVE_ROOT="${SAVE_ROOT_BASE}/instance${PARAMS}${r}"
  TARGET_DIR_NAME="${TARGET_CONCEPT//, /_}"

  echo "========== [r=${r}] Erasing ${TARGET_CONCEPT} with params=${PARAMS} =========="
